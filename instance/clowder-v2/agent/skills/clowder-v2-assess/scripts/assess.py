...
def _dependency_versions(repo: Path, files: dict[Path, str]) -> dict[str, list[str]]:
    ...
    for path, text in files.items():
        ...
        if not (
            name in {"pyproject.toml", "uv.lock", "poetry.lock", "pipfile.lock"}
...
def analyze(repo: Path, phase: str) -> Assessment:
    ...
    python_clowder = bool(assessment.dependency_versions) or any(
        path.suffix == ".py" and "DependencyEndpoints" in text
    ...
    if helper_calls:
        ...
        assessment.errors.append(
            Finding("No Python Clowder V2 helper calls found after migration.")
        )
